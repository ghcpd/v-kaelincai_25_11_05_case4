...
GRADER_PORT = int(os.environ.get("GRADER_PORT", "7000"))
...
def start_mocks_and_grader(scorer_config):
    ...
    for i, p in enumerate(SCORER_PORTS):
        ...
        env["PORT"] = str(p)
        proc = start_process(["python", str(ROOT / "Project_B_Deterministic" / "mocks" / "scorer_mock.py")], env=env)
        procs.append(proc)
    # start grader
    env = os.environ.copy()
    env["GRADER_PORT"] = str(GRADER_PORT)
    env["SCORER_PORTS"] = ",".join(SCORER_PORTS)
    proc_grader = start_process(["python", str(ROOT / "Project_B_Deterministic" / "src" / "grading_service.py")], env=env)
...
def compute_metrics(runs, timings=None):
    ...
    if timings:
        res["avg_latency"] = sum(timings)/len(timings)
    return res


def test_post_deterministic_experiment():
    ...
    out_file = ROOT / 'results' / 'results_post.json'
    out_file.parent.mkdir(exist_ok=True, parents=True)
    with open(out_file, 'w') as f:
        json.dump(results, f, indent=2)

    # Assert deterministic outputs in Project B: all tests should have zero flakiness
    flakiness_sum = sum(1 for v in results.values() if v['metrics']['flakiness_rate'] > 0)
    assert flakiness_sum == 0, "Expected deterministic behavior in Project B tests"
