"""
Scorer mock service for simulating async scoring with configurable delays and failures.
This mock intentionally introduces non-deterministic delays and failures to simulate real scorer behavior.
"""
import asyncio
import random
import logging
...
class ScorerMock:
    ...
    async def score(self, answer: str, retry_attempt: int = 0) -> Dict[str, Any]:
        """
        Score an answer with simulated latency and potential failure.
        
        This intentionally introduces non-determinism:
        - Random latency means out-of-order completions
        - Intermittent failures trigger retries
        - No idempotency - retries may overwrite previous results
        ...
        # Simulate variable latency (RACE CONDITION SOURCE)
        ...
        logger.info(f"[{call_id}] SCORED: question={self.question_id}, score={score}, latency_ms={latency:.1f}")
        return result
    ...
    def _compute_score(self, answer: str) -> int:
        ...
        if not answer or len(answer.strip()) == 0:
            return 0
        # Reward longer answers (simplified demo logic)
...
class ScorerRegistry:
    ...
    def register_scorer(
        ...
        # Use deterministic seed offset for each scorer if global seed provided
        scorer_seed = None
    ...
    async def score_answer(
        ...
        return await self.scorers[question_id].score(answer, retry_attempt)
