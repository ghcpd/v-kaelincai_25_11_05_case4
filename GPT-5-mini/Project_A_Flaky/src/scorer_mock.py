...
# Simple HTTP mock scorer that simulates variable latency and intermittent failures.
class ScorerHandler(BaseHTTPRequestHandler):
    # config will be attached to server
    ...
    def do_POST(self):
        ...
        if random.random() < cfg.get('fail_rate',0):
            ...
            return
        # produce deterministic per-question score based on answer text but include noise in flaky version
        base = sum((ord(c)%5) for c in req.get('answer',''))
        # Flaky: sometimes return different score even for same answer
    ...
    def log_message(self, format, *args):
        # silence standard logging to keep outputs tidy
        return
